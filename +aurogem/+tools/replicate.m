...
for track_id = 1:num_tracks
    ...
    if opts.save_data
        save('data\reps.mat', 'x2_traj_rep', 'x3_traj_rep', 'd2_traj_rep', 'd3_traj_rep')
    end
    ...
    if opts.show_plots
        ...
        set(gcf, 'Position', ful)
        t = tiledlayout(1, 1);
        t.Title.String = sprintf('%sData Smoothing', title_pfx);
        t.Title.FontSize = 1.5*fts;

        nexttile
        hold on
        plot(x3_traj*scl.x, d2_traj*scl.d, 'r')
        plot(x3_traj*scl.x, d3_traj*scl.d, 'b')
        if flow_driven
            ...
            plot(x3_traj_old*scl.x, track.fac*scl.d, 'Color', [1, 0.5, 0.5])
            plot(x3_traj_old*scl.x, zeros(size(track.fac)), 'Color', [1, 0.5, 0.5])
        end
        xlabel(lbl.y); ylabel(sprintf('Data (%s)', unt.d))
    ...
    if opts.show_plots
        figure
        t = tiledlayout(1, 1);
        t.Title.String = sprintf('%sInterpolated Data', title_pfx);
        t.Title.FontSize = 1.5*fts;
        set(gcf, 'Position', ful)
        vs = 2;
        ...
        quiver(X2*scl.x, X3*scl.x, d2_int*scl.d*vs, d3_int*scl.d*vs, 0, '.-k')
        quiver(x2_traj*scl.x, x3_traj*scl.x, ...
            d2_traj*scl.d*vs, d3_traj*scl.d*vs, 0, '.-g')
        xlim(lim.x); ylim(lim.y)
        xlabel(lbl.x); ylabel(lbl.y)
        colormap(gca, colorcet(clm.arc))
        clb = colorbar;
...
if (opts.show_plots || opts.save_plots(2)) && not(flow_driven)
    figure
